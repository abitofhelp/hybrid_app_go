...
- **Components**:
  - `Version` - full semantic version string (e.g., "1.0.2")
  - `Major`, `Minor`, `Patch` - numeric components
  - `Prerelease`, `BuildMetadata` - optional identifiers
  - `IsPrerelease()`, `IsStable()`, `IsDevelopment()` - version predicates
- **Generation**: Auto-generated by `release.py prepare <version>`
- **Usage**: Presentation layer imports for CLI `--help` version display
